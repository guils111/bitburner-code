import { HACK_PERCENT } from "../../lib/constants.js";
...
const IS_SHARE_ENABLED_ON_WAIT = false;
const RAM_RESERVE = 0;
...
async function shotgun(ns) {
  ...
  let target;
  if (await ns.prompt("Override target?")) {
  ...
  while (true) {
    ...
    if (Date.now() > timeToSwitchTargets || runTime < 2000) {
      target = findBestTarget(ns, false, target);
      await prepareServer(ns, target);
