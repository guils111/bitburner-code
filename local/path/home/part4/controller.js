...
class ContinuousBatcher {
	...
	log() {
		...
		ns.clearLog();
		ns.print(`Hacking ~\$${ns.format.number(metrics.maxMoney * metrics.greed * metrics.chance / (4 * metrics.spacer) * 1000)}/s from ${metrics.target}`);
		ns.print(`Status: ${isPrepped(ns, this.#target) ? "Prepped" : "Desynced"}`);
		ns.print(`Security: +${metrics.minSec - metrics.sec}`);
		ns.print(`Money: \$${ns.format.number(metrics.money, 2)}/${ns.format.number(metrics.maxMoney, 2)}`);
		ns.print(`Greed: ${Math.floor(metrics.greed * 1000) / 10}%`);
		ns.print(`Ram available: ${ns.format.ram(ramNet.totalRam)}/${ns.format.ram(ramNet.maxRam)}`);
...
class Job {
	/**
	 * @param {string} type
	 * @param {{ end: any; times: { [x: string]: any; }; target: any; threads: { [x: string]: any; }; port: any; }} metrics
	 * @param {number} batch
	 */
...
class RamNet {
	...
	/**
	 * @param {{ cost: number; server: any; }} job
	 */
	assign(job) {
	...
	/**
	 * @param {{ server: any; cost: number; }} job
	 */
	finish(job) {
